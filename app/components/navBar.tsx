...
const NavBar = () => {
  ...
  return (
    <nav
      className={`bg-falightgray fixed w-full z-10 justify-start items-start duration-300 ease-out ${
    ...
    >
      <div className="bg-fablue justify-start items-start mb-1">
        <div className="px-10 flex justify-between h-14">
          ...
          {/* Mobile Menu */}
          <div
            className={`flex flex-col w-full md:flex-row md:w-auto md:flex ${
              isOpen ? "block" : "hidden"
            }`}
          >
            <ul className="flex flex-col md:flex-row">
              {navItems.map((item) => (
                <li key={item.name} className="md:flex items-center">
                  <Link
                    href={item.path}
                    className={`h-full px-6 py-2 duration-300 hover:bg-falightgray hover:text-fablue flex items-center font-bold ${
                      pathName === item.path
                        ? "bg-falightgray text-fablue"
                        : "bg-fablue text-falightgray"
                    }`}
                  >
                    <span className="flex items-center">
                      {item.name}
                      {item.name === "CCTV" && notification.length > 0 && (
                        <span className="flex justify-center items-center ml-2 text-falightgray rounded-full bg-red-600 text-xs p-1 w-6 h-6">
                          {`${notification.length}`}
                        </span>
                      )}
                    </span>
